...
class DockerBoundaryArtifactTests(unittest.TestCase):
    ...
    def test_harness_exercises_check_apply_idempotence_refusals_and_cleanup(self):
        ...
        self.assertIn("GitHub-hosted auxiliary PCI interface", harness)
        self.assertIn("name ~ /^enP[0-9]+s[0-9]+$/", harness)
        self.assertIn("fail docker_managed_interface_detected", harness)
