...
| F1.2c Network enforcement | isolamento/egress/service discovery v4/v6 | `ADR_ACCEPTED` | contrato Q20/Q34 local-static `PASS`; DEC-008 seleciona `DOCKER-USER`, bridges internas e egress proxy-only; implementação, matriz dinâmica allow/deny e rollback continuam `PENDING`; bloqueia primeiro container |
