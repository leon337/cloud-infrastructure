...
| Network/egress/service discovery | **PARTIAL — NODE-01 DESIRED STATE CI PASS** | Docker `iptables-nft` + `DOCKER-USER`, bridges internas por sandbox, DNS por escopo e egress proxy-only | base fail-closed v4/v6 está ativa no NODE-01; serviços passaram lifecycle commit-bound no run `32131461110`; apply real ainda pendente. |
