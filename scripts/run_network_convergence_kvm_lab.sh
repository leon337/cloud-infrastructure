...
(( mem_kib >= 2 * 1024 * 1024 )) || refuse insufficient_host_memory
free_kib=$(df -Pk "${TMPDIR:-/tmp}" | awk 'NR==2 {print $4}')
[[ $free_kib =~ ^[0-9]+$ ]] || refuse host_disk_probe_failed
(( free_kib >= 3 * 1024 * 1024 )) || refuse insufficient_host_disk
...
RUN_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/mcf-network-convergence-kvm.XXXXXXXX")
readonly RUN_ROOT
...
cleanup() {
  case $RUN_ROOT in
    "${TMPDIR:-/tmp}"/mcf-network-convergence-kvm.*) ;;
...
GUEST_HARNESS_COMMAND="set -Eeuo pipefail; \
  ...
  NETWORK_CONVERGENCE_KVM_CONFIRM=MCF_NETWORK_CONVERGENCE_KVM_ONLY \
    scripts/test_network_convergence_recovery_vm.sh"
